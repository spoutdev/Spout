...
public class SpoutTexture extends Texture {
	...
	public SpoutTexture(BufferedImage image) {
		ByteBuffer data;
		texture = ((SpoutClient) Spout.getEngine()).getRenderer().getGL().createTexture();
		data = CausticUtil.getImageData(image, Format.RGBA);
		data.flip();
		texture.setImageData(data, image.getWidth(), image.getHeight());
	}
	...
	public void setImageData(ByteBuffer buffer, int width, int height) {
		texture.setImageData(buffer, width, height);
	}
