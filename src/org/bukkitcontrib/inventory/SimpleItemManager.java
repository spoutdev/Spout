...
import org.bukkitcontrib.packet.PacketItemName;
    ...
    public void resetName(Material item, byte data) {
        ...
        if (customNames.containsKey(info)) {
            ...
            for (Player player : Bukkit.getServer().getOnlinePlayers()) {
                if (player instanceof ContribCraftPlayer){
                    if (((ContribCraftPlayer)player).getVersion() > 8) {
                        ((ContribCraftPlayer)player).sendPacket(new PacketItemName(info.id, info.data, "[reset]"));
                    }
                }
            }
        }
    ...
    public void onPlayerJoin(ContribPlayer player) {
        if (((ContribCraftPlayer)player).getVersion() > 8) {
            ...
            while (i.hasNext()) {
                ...
                ((ContribCraftPlayer)player).sendPacket(new PacketItemName(e.getKey().id, e.getKey().data, e.getValue()));
            }
