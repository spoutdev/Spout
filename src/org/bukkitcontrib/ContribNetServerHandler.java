...
public class ContribNetServerHandler extends NetServerHandler {
	...
	public void a(Packet101CloseWindow packet) {
		...
		InventoryCloseEvent event = new InventoryCloseEvent(
				(Player) this.player.getBukkitEntity(), inventory,
	...
	public void a(Packet106Transaction packet) {
		...
		Short oshort = this.n.get(Integer
				.valueOf(this.player.activeContainer.windowId));

		if (oshort != null && packet.b == oshort.shortValue()
				&& this.player.activeContainer.windowId == packet.a
	...
	public void a(Packet102WindowClick packet) {
		if (this.player.activeContainer.windowId == packet.a
				...
				if (inventory instanceof CraftingInventory) {
					...
					if (type == InventorySlotType.RESULT) {
						InventoryCraftEvent craftEvent = new InventoryCraftEvent(
								this.getPlayer(), crafting,
								this.activeLocation, type, packet.b, matrix,
								craftResult, cursor, packet.c == 0, packet.f);
			...
			if (clickSuccessful) {
				...
				this.n.put(
						Integer.valueOf(this.player.activeContainer.windowId),
	...
	public boolean handleInventoryClick(Packet102WindowClick packet,
		...
		if (!(this.player.activeContainer instanceof ContainerPlayer)
			...
			event = new InventoryPlayerClickEvent(this.getPlayer(), this
					.getPlayer().getInventory(), type, slot, cursor,
					activeSlot, click == LEFT_CLICK, packet.f, activeLocation);
		} else {
			event = new InventoryClickEvent(this.getPlayer(), inventory, type,
					slot, cursor, packet.b, click == LEFT_CLICK, packet.f,
					activeLocation);
		}
		...
		// NOTE: Successful means that its successful as-is; thus, only becomes
		// true for default behaviour

		switch (result) {
		case DEFAULT:
			itemstack = this.player.activeContainer.a(packet.b, packet.c,
					packet.f, this.player);
		...
		case DENY:
			if (packet.b != -999) { // Only swap if target is not OUTSIDE
				...
				if (event.getCursor() != null) {
					setActiveSlot(packet.b, itemstack);
					// cursorstack = new
					// ItemStack(event.getCursor().getTypeId(),
					// event.getCursor().getAmount(),
					// event.getCursor().getDurability());
		...
		case ALLOW: // Allow the placement unconditionally
			if (packet.b == -999) { // Clicked outside, just defer to default
				...
				if (click == LEFT_CLICK
					...
					// Left-click full slot with full cursor of same item; merge
					// stacks
					...
					cursorstack = null;
						...
						|| (itemstack != null && cursorstack != null && !itemstack
								.doMaterialsMatch(cursorstack))) {
					// Either left-click, or right-click full slot with full
					// cursor of different item; just swap contents
					ItemStack temp = itemstack;
					itemstack = cursorstack;
					cursorstack = temp;
				} else if (click == RIGHT_CLICK) { // Right-click with either
													// slot or cursor empty
		...
		try {
			if (container instanceof ContainerChest) {
				...
				return new ContribCraftInventory(
						(IInventory) a.get((ContainerChest) container));
			}
			if (container instanceof ContainerPlayer) {
				return new ContribCraftInventoryPlayer(this.player.inventory,
			...
			if (container instanceof ContainerFurnace) {
				...
				a.setAccessible(true);
				return new ContribCraftInventory(
						(TileEntityFurnace) a.get((ContainerFurnace) container));
			}
			if (container instanceof ContainerDispenser) {
				Field a = ContainerDispenser.class.getDeclaredField("a");
				a.setAccessible(true);
				return new ContribCraftInventory(
						(TileEntityDispenser) a
								.get((ContainerDispenser) container));
			}
