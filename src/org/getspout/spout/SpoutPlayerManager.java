...
public class SpoutPlayerManager extends net.minecraft.server.PlayerManager { // Spout
	...
	public SpoutPlayerManager(MinecraftServer minecraftserver, int i, int j) { // Spout
		super(minecraftserver, i, j); // Spout
		if (j > 15) {
	...
	private SpoutPlayerInstance a(int i, int j, boolean flag) { // Spout
		long k = (long) i + 2147483647L | (long) j + 2147483647L << 32;
		SpoutPlayerInstance playerinstance = (SpoutPlayerInstance) this.b.a(k); // Spout

		if (playerinstance == null && flag) {
			playerinstance = new SpoutPlayerInstance(this, i, j); // Spout
	...
	public void flagDirty(int i, int j, int k) {
		int l = i >> 4;
		int i1 = k >> 4;
		SpoutPlayerInstance playerinstance = this.a(l, i1, false); // Spout
	...
	public void removePlayer(EntityPlayer entityplayer) {
		...
		for (int k = i - this.f; k <= i + this.f; ++k) {
			for (int l = j - this.f; l <= j + this.f; ++l) {
				SpoutPlayerInstance playerinstance = this.a(k, l, false); // Spout
				...
				for (int k1 = i - this.f; k1 <= i + this.f; ++k1) {
					for (int l1 = j - this.f; l1 <= j + this.f; ++l1) {
						...
						if (!this.a(k1 - i1, l1 - j1, i, j)) {
							SpoutPlayerInstance playerinstance = this.a(k1 - i1, l1 - j1, false); // Spout
	...
	static PlayerList a(SpoutPlayerManager playermanager) { // Spout
		return playermanager.b;
	}

	static List b(SpoutPlayerManager playermanager) { // Spout
