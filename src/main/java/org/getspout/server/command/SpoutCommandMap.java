...
public class SpoutCommandMap extends SimpleCommandMap {
    ...
    public SpoutCommandMap(final Server server) {
        super(server);
        this.server = server;
    }
    ...
    public static void initGlowPermissions(Server server) {
        ...
        parent = new Permission(SpoutCommand.PERM_PREFIX, "Gives access to all Glowstone commads");
    ...
    public void registerAll(Collection<SpoutCommand> commands) {
        for (SpoutCommand command : commands) {
            register(command);
        }
    }
    ...
    public void registerPermissions(SpoutCommand command) {
        PluginManager pm = server.getPluginManager();
        Permission child = new Permission(SpoutCommand.PERM_PREFIX + "." + command.getName(), command.getPermissionDefault());
    ...
    public void registerAllPermissions() {
        for (Command command : knownCommands.values()) {
            if (command instanceof SpoutCommand) {
                registerPermissions((SpoutCommand) command);
            }
        }
    }
    ...
    public Set<String> getCommandPermissions() {
        ...
        for (Command command : knownCommands.values()) {
            ...
            if (command instanceof SpoutCommand) {
                for (Permission perm : ((SpoutCommand) command).registerPermissions(parent.getName())) {
